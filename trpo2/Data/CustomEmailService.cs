...
namespace trpo2.Data
{
    ...
    {
        CustomErrorMessage SendMessage(ApplicationTicket applicationTicket, string selectedCountry, string selectedCity, string selectedHotel);
    }

    public class CustomEmailService : IEmailService
    {
        public CustomErrorMessage SendMessage(ApplicationTicket applicationTicket, string selectedCountry, string selectedCity, string selectedHotel)
        {
            ...
            try
            {
                ...
                mail.To.Add(applicationTicket.Email);
                ...
                var smtp = new SmtpClient("smtp.mail.ru", 465)
                {
                    Credentials = new NetworkCredential("bakasov_a@internet.ru", "YO6iPUt^ipu1"),
                    EnableSsl = true
                };
                ...
                {
                    ErrorMessage = $"Письмо {applicationTicket.Email} успешно отправлено.",
                    Success = true
                };
            }
            catch (Exception ex)
